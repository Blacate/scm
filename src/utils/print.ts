...
const printList = (sshClients: SshClient[]) => {
  ...
  sshClients.forEach(sshClient => {
    ...
    maxLength.category = Math.max(maxLength.category, sshClient.category.length)
  })
  ...
  console.log('Alias'.padEnd(aliasLength), 'Category'.padEnd(categoryLength), 'Server'.padEnd(serverLength), 'User'.padEnd(userLength), 'Port'.padEnd(portLength));
  console.log('-'.repeat(aliasLength + categoryLength + userLength + portLength + categoryLength));
  sshClients.forEach(sshClient => {
    console.log(sshClient.alias.padEnd(aliasLength), sshClient.category.padEnd(categoryLength), sshClient.server.padEnd(serverLength), sshClient.user.padEnd(userLength), sshClient.port.toString().padEnd(portLength));
  });
}
